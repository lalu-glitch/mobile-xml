...
class _AuthGuardState extends State<AuthGuard> {
  ...
  void initState() {
    super.initState();
    log('[AuthGuard] initState: Scheduling _checkAppFlow.');
  ...
  Future<void> _checkAppFlow() async {
    log('[AuthGuard] _checkAppFlow: Starting flow check...');
  ...
  @override
  void dispose() {
    log('[AuthGuard] dispose: Guard is being disposed.');
    _timer?.cancel();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_loading) {
      log('[AuthGuard] build: Showing Loading Indicator (main)');
    ...
    if (!_isLoggedIn && !_showOnboarding) {
      log('[AuthGuard] build: Showing SizedBox.shrink() (blank screen)');
      return const SizedBox.shrink();
    }

    // Ternary ini sekarang lebih sederhana
    if (_isLoggedIn) {
      log('[AuthGuard] build: Showing widget.child (App Content)');
      return widget.child;
    } else {
      // Bagian 'else' ini kemungkinan tidak akan pernah tercapai
      // karena sudah ditangani oleh 'if (_loading)' atau 'if (!_isLoggedIn ...)'
      log('[AuthGuard] build: Showing Loading Indicator (fallback)');
      return const Scaffold(
        body: Center(child: CircularProgressIndicator(color: kOrange)),
      );
    }
